...
//update the event in the table "events"
$stmt = $mysqli->prepare("update events set title=?,tag=?,date=? where id=? and username=?");
if(!$stmt){
	echo json_encode(array(
		"success" => false,
		"message" => "Inputs are illegal!"
...
$stmt->bind_param('sssss', $title,$tag,$date,$id,$username);
