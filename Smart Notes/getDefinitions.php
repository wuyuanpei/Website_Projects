...
if($seq=="date")
  $stmt = $mysqli->prepare("select id, example ,term, definition, remark, course, tag, date, title from definitions where username=? order by date");
else if($seq=="tag")
  $stmt = $mysqli->prepare("select id, example ,term, definition, remark, course, tag, date, title from definitions where username=? order by tag");
else
  $stmt = $mysqli->prepare("select id, example ,term, definition, remark, course, tag, date, title from definitions where username=? order by course");
...
if(!$stmt){
    ...
    $stmt->execute();
  	...
  	$stmt->bind_result($id, $example ,$term, $definition, $remark, $course, $tag, $date, $title);
}
